 ...
 * Problem Set 4
 *
 * Copies a file, cleans the red out of it, and returns it so it is an usable image
 */
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0, biHeight = abs(bi.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi.biWidth; j++)
        {
            ...
            //if the pixel is all red, turn it to white
            if (triple.rgbtRed == 0xff && triple.rgbtGreen == 0x00 && triple.rgbtBlue == 0x00 )
            {
                triple.rgbtRed = 0x00;
            }
            
            //remove all the black and make it white
