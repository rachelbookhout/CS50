...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    //set up our pointers to have stuff added to them
    *length = 0;
    *content = NULL;
    ...
    for(int c = fread(buffer, 1, 512, file); c != 0; c = fread(buffer, 1, 512, file))
    {
        ...
        *content = realloc(*content, *length + c);
        
        // check for errors reallocating memory
        if (content == NULL)
        {
            return false;
        }
        
        //copy the stuff from bufer into content
...
const char* lookup(const char* path)
{
      ...
      //turn extension into lower case letters if not already to check it
      for (int i = 0; i < strlen(extension); i++)
      {
          extension[i] = tolower(extension[i]);
      }
      
      //check what our extension is so we can route accordingly
      if (extension != NULL)
        {
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // get the url we will be going to, both the absolute path and query
    char* target = strtok(NULL, " ");
    
    //check that the first value in our absolute path is not a /
    if(target[0] != '/')
    {
        // error
        error(501);
    ...
    {
        // checks if any character in the request-target is "
        if(target[i] == '"')
        {
            error(400);
            return false;
    ...
    //get the HTTP version at the end of our line
   ...
   // further mince target (our url) to get the query
    char* query_path = strchr(target,'?');
    
    if (query_path == NULL)
    {
        //add target(our url) to abs_path
        //add a null terminator to query to indicate there isn't a value
    ...
    {
        //copy the query_path straight to copy after adding the null terminator
        query_path[strlen(query_path)] = '\0';
        strcpy(query,query_path);

        //iterate over target and put each character into abs_path along with the null terminator
        int i;
        for(i = 0; target[i] != '?'; i++)
        {
            abs_path[i] = target[i];
